...
def main() -> None:
    ...
    # --- left: recall ---
    axL.plot(x, nap_rec, "-o", color=BLUE, lw=2.4, ms=6, label="NapLoRA (weights + retrieval)")
    axL.plot(x, van_rec, "-o", color=GREY, lw=2.0, ms=5, label="Vanilla (8k context window)")
    axL.set_ylim(-5, 108)
    axL.set_ylabel("needle recall (%)", color=DIM)
    axL.set_title("Recall holds as the conversation grows", color=INK)
    axL.text(window, 6, " 8k window", color=GREY, fontsize=9, rotation=90, va="bottom", ha="right")
    axL.text(paper, 6, " D2L single-encode ~4×window", color="#9a7a30", fontsize=9,
             rotation=90, va="bottom", ha="right")
    axL.legend(loc="lower left", frameon=True, fontsize=10, facecolor="white", edgecolor="#e7e4dd")

    # --- right: cost ---
    axR.plot(x, nap_tok, "-o", color=BLUE, lw=2.4, ms=6, label="NapLoRA prompt")
    axR.plot(x, van_tok, "-o", color=GREY, lw=2.0, ms=5, label="Vanilla prompt")
    axR.set_ylabel("query-time prompt tokens  (≈ KV-cache cost)", color=DIM)
    axR.set_title("Inference cost stays flat", color=INK)
    # annotate the gap at the largest point
    xr, ny, vy = x[-1], nap_tok[-1], van_tok[-1]
    if ny > 0:
        axR.annotate(f"{vy/ny:.0f}× less\ncontext", xy=(xr, ny), xytext=(xr * 0.28, vy * 0.45),
                     color=BLUE, fontsize=10, ha="center", fontweight="bold",
                     arrowprops=dict(arrowstyle="->", color=BLUE, lw=1.4))
    axR.legend(loc="center left", frameon=True, fontsize=10, facecolor="white", edgecolor="#e7e4dd")
